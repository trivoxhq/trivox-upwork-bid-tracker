...
import { importLeadRows } from "@/lib/leads/spreadsheet";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await importLeadRows(upload.parsed.headers, upload.parsed.rows, actor, users);

    return NextResponse.json({
      success: true,
      message: `Imported ${result.created} lead(s). ${result.failed} row(s) failed.`,
      result,
    });
  } catch {
    return NextResponse.json(
