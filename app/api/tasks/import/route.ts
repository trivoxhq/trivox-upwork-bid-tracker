...
import { importTaskRows } from "@/lib/tasks/spreadsheet";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await importTaskRows(upload.parsed.headers, upload.parsed.rows, actor, users);

    return NextResponse.json({
      success: true,
      message: `Imported ${result.created} task(s). ${result.failed} row(s) failed.`,
      result,
    });
  } catch {
    return NextResponse.json(
